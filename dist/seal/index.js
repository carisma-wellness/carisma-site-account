...
export function seal(payload, secret, rpId) {
...
function tryDecrypt(iv, ct, tag, key, aadBuf) {
    try {
 ...
 */
export function unseal(token, keys, rpId) {
    ...
    const secrets = [keys.primary, keys.prev].filter((s) => typeof s === "string" && s.length > 0);
