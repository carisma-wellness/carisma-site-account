export function step3InitialState(input) {
    return {
...
export function step3Reducer(state, action) {
    switch (action.type) {
        case "SUBMIT_DETAILS":
 ...
 */
export function step3View(state) {
    if (!state.accountLoginEnabled)
        return "guest-form";
    if (state.session.signedIn)
        return state.editing ? "confirm-edit" : "confirm";
    return state.guestDoorOpen ? "guest-form" : "doors";
}
/** True only once SUBMIT_DETAILS has written a guest (W-10). */
export function isReadyToCheckout(state) {
...
export function assertReadyToPay(state) {
    if (!isReadyToCheckout(state))
        throw new NotReadyToPayError();
}
