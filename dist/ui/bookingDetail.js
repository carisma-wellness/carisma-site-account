...
function mapHrefFor(lat, lng, address) {
    const la = Number(lat);
    const ln = Number(lng);
...
export function readActions(raw) {
...
export function buildBookingDetailModel(body, id) {
    ...
    const a = inner;
    const venue = (a.venue && typeof a.venue === "object" ? a.venue : {});
    ...
    const flat = Array.isArray(a.services) ? a.services : [];
    const participants = Array.isArray(a.participants)
        ? a.participants
        : [];
    const nested = participants.flatMap((p) => Array.isArray(p.services) ? p.services : []);
    const services = [...flat, ...nested].map((s) => ({
    ...
    return {
        ...
        brand,
        venue: str(venue.locationName) || str(venue.name),
        address,
        ...
        due,
        actions,
        policyText: str(a.policyText) || str(a.cancellationPolicy?.policyText),
...
export function isActiveBooking(m) {
 ...
 */
export function bookingPrimary(m) {
    if (canPay(m))
        return "pay";
    if (canMove(m))
        return "reschedule";
...
export function rebookHref(bookHref, serviceId) {
    const base = bookHref || "/";
    if (!serviceId)
        return base;
...
function chipFor(m) {
    if (m.status === "PENDING")
        return { label: m.statusLabel, tone: "warn" };
    if (isActiveBooking(m) && canPay(m))
...
const ICON = (d, size = 18) => `<svg aria-hidden="true" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" ` +
...
export function bookingTreatment(m) {
    return treatmentOf(m);
}
function btn(action, label, kind, id, aria) {
    return (`<button type="button" class="cw-btn cw-btn--${kind}" data-cw-action="${action}" data-cw-appt="${escapeHtml(id)}"` +
        (aria ? ` aria-label="${escapeHtml(aria)}"` : "") +
        `>${escapeHtml(label)}</button>`);
}
function heroHTML(m, o) {
    ...
    return (`<section class="cw-bk-hero" aria-label="When">` +
...
function factsHTML(m) {
    const fact = (label, value) => `<div class="cw-fact"><dt class="cw-label">${label}</dt><dd class="cw-fact__value">${value}</dd></div>`;
    ...
    return (`<dl class="cw-facts cw-bk-facts">` +
...
function moneyHTML(m, primary) {
    const pay = primary === "pay"
    ...
    const row = (label, value, cls = "") => `<div class="cw-bk-money__row${cls}"><span>${escapeHtml(label)}</span><span ${M}>${escapeHtml(value)}</span></div>`;
    if (m.due > 0) {
        // A missed visit's fee is the whole story; the original ticket price
        // beside it only invites the wrong sum.
        const rows = m.status === "NO_SHOW"
            ? ""
...
function actionsHTML(m, primary, o) {
    const a = m.actions;
    ...
    const secondary = (action) => {
        if (action === "reschedule")
            return btn("reschedule", "Reschedule", "secondary", m.id, `Reschedule ${spoken}`);
        if (action === "confirm")
            return btn("confirm", "Confirm I'm coming", "secondary", m.id, `Confirm you're coming to ${spoken}`);
        return `<a class="cw-btn cw-btn--secondary" href="${escapeHtml(rebookHref(o.bookHref, m.serviceId))}">Book again</a>`;
    };
    let cta = "";
    if (primary === "reschedule")
        cta = btn("reschedule", "Reschedule", "primary", m.id, `Reschedule ${spoken}`);
    if (primary === "confirm")
        cta = btn("confirm", "Confirm I'm coming", "primary", m.id, `Confirm you're coming to ${spoken}`);
    if (primary === "rebook")
        cta = `<a class="cw-btn cw-btn--primary" href="${escapeHtml(rebookHref(o.bookHref, m.serviceId))}">Book again</a>`;
    if (primary !== "reschedule" && canMove(m))
        buttons.push(secondary("reschedule"));
    if (primary !== "confirm" && canConfirmIt(m))
        buttons.push(secondary("confirm"));
    // Past visit with money owed: Pay leads, Book again follows.
    if (primary !== "rebook" && a.canRebook && !isActiveBooking(m))
        buttons.push(secondary("rebook"));
    // Why it cannot move, in the server's words, with the phone a link.
    const reason = isActiveBooking(m) && !a.canReschedule && a.reason
    ...
    const deadline = canMove(m) && Number.isFinite(closes) && closes > nowMs && closes - nowMs < 48 * HOUR
        ? `<p class="cw-reason cw-bk-deadline">${CLOCK_ICON}<span>You can move this online until ${escapeHtml(instantWords(a.rescheduleClosesAt))}.</span></p>`
        : "";
    const quiet = [];
    if (isActiveBooking(m) && m.startIso) {
        quiet.push(`<button type="button" class="cw-btn cw-btn--quiet cw-bk-quiet__item" data-cw-action="calendar" data-cw-appt="${escapeHtml(m.id)}">${CAL_ICON}Add to calendar</button>`);
        if (o.wallet?.apple) {
            quiet.push(`<button type="button" class="cw-btn cw-btn--quiet cw-bk-quiet__item" data-cw-action="wallet" data-cw-wallet="apple" data-cw-appt="${escapeHtml(m.id)}">${WALLET_ICON}Add to Apple Wallet</button>`);
        }
        if (o.wallet?.google) {
            quiet.push(`<button type="button" class="cw-btn cw-btn--quiet cw-bk-quiet__item" data-cw-action="wallet" data-cw-wallet="google" data-cw-appt="${escapeHtml(m.id)}">${WALLET_ICON}Add to Google Wallet</button>`);
        }
    }
    const cancel = isActiveBooking(m) && a.canCancel
        ...
        : "";
    if (!cta && !buttons.length && !reason && !deadline && !quiet.length && !cancel)
        return "";
    return (`<div class="cw-bk-actions">` +
...
export function policyLead(m, preview) {
    const a = m.actions;
    if (!a.canCancel)
        return "";
    if (a.cancelIsFree && a.freeCancelEndsAt)
        return `Free to cancel until ${instantWords(a.freeCancelEndsAt)}.`;
    if (a.cancelIsFree)
        return "Free to cancel.";
    if (!preview)
        return "";
    if (preview.chargeAmount > 0)
        return `Cancelling now costs ${eur(preview.chargeAmount)}.`;
    if (preview.forfeitAmount > 0)
        return `Cancelling now keeps ${eur(preview.forfeitAmount)} of what you've paid.`;
    return "Free to cancel.";
}
function policyHTML(m, o) {
    // Only while the visit is ahead: a policy about changing a finished visit
    // is noise at best and reads like a threat at worst.
    if (!m.policyText || !isActiveBooking(m))
        return "";
    const lead = policyLead(m, o.preview);
    return (`<section class="cw-section cw-bk-policy" aria-labelledby="cw-bk-policy-title">` +
 ...
 */
export function bookingViewParts(m, o = {}) {
    if (!m.found || m.isMedical) {
        return {
            title: "We couldn't find that booking",
            lede: backLink(),
            body: `<section class="cw-empty cw-empty--quiet cw-rise">` +
    ...
    const lede = backLink() + (showBrand ? `<span class="cw-label cw-bk-brand" ${M}>${escapeHtml(m.brand)}</span>` : "");
    const money = moneyHTML(m, primary);
    const actions = actionsHTML(m, primary, o);
    const body = `<div class="cw-bk${primary ? " has-cta" : ""}" data-cw-qc="${BOOKING_DETAIL_QC}" data-cw-status="${escapeHtml(m.status)}">` +
...
export function bookingSkeletonHTML() {
    const line = (w, h = 12) => `<span class="cw-skel cw-skel--line" style="width:${w};height:${h}px"></span>`;
    return (`<p class="cw-vh">Loading your booking…</p>` +
...
export function bookingDetailHTML(m, o = {}, shell = {}) {
