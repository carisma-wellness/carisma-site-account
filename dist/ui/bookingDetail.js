...
function moneyBlock(m) {
    if (m.total <= 0 && m.paid <= 0 && m.due <= 0)
        return "";
    const row = (label, value, strong = false) => `<div class="carisma-portal__row${strong ? " is-strong" : ""}">` +
        `<span>${escapeHtml(label)}</span><span ${M}>${escapeHtml(value)}</span></div>`;
    const lines = row("Treatment", eur(m.total)) +
    ...
    const pay = m.actions.canPayBalance
        ...
        : "";
    return (`<section class="carisma-portal__block"><h2 class="carisma-portal__eyebrow">Payment</h2>` +
        lines +
...
function actionsBlock(m) {
    const a = m.actions;
    const btn = (action, label, primary = false) => `<button type="button" class="carisma-portal__btn${primary ? " is-primary" : ""}" ` +
    ...
    const keep = a.canConfirm || a.canReschedule || a.canCancel
...
function policyBlock(m) {
    ...
    const freeUntil = a.freeCancelEndsAt && a.cancelIsFree
    ...
    return (`<section class="carisma-portal__block"><h2 class="carisma-portal__eyebrow">Changing this booking</h2>` +
        freeUntil +
        policy +
        `</section>`);
}
export function bookingDetailHTML(m) {
    if (!m.found) {
        return (`<main class="carisma-portal" data-cw-qc="${BOOKING_DETAIL_QC}">` +
    ...
    const withWhom = m.staffName
        ...
        : "";
    return (`<main class="carisma-portal" data-cw-qc="${BOOKING_DETAIL_QC}" data-cw-status="${escapeHtml(m.status)}">` +
