...
export function memberDoor(cfg, next) {
    if (cfg.accountLogin) {
...
export function memberFallbackHTML(cfg) {
    const brand = escapeHtml(cfg.brandName);
    const contactBits = [];
    if (cfg.phoneHref) {
        contactBits.push(`<a href="${escapeHtml(cfg.phoneHref)}">${escapeHtml(cfg.phoneLabel || cfg.phoneHref)}</a>`);
    }
    if (cfg.whatsappHref) {
        contactBits.push(`<a href="${escapeHtml(cfg.whatsappHref)}">${escapeHtml(cfg.whatsappLabel || "WhatsApp")}</a>`);
    }
    const contact = contactBits.length
        ? `<p class="carisma-member-fallback__contact">You can reach us on ${contactBits.join(" or ")}.</p>`
        : "";
    return (`<main class="carisma-member-fallback">` +
...
//# sourceMappingURL=memberDoor.js.map