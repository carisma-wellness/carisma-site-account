 ...
 */
export function dateWords(dateStr, style) {
    const p = ymd(dateStr);
    if (!p)
        return dateStr;
...
export function instantWords(iso, style = "short", timeZone = MALTA) {
    ...
    try {
        ...
        const clock = new Intl.DateTimeFormat("en-GB", { timeZone, hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(ms));
        return `${day}, ${clock}`;
    }
    catch {
        return "";
...
export function clockOf(iso, timeZone = MALTA) {
    const ms = Date.parse(iso);
    if (!Number.isFinite(ms))
        return "";
    try {
 ...
 */
export function buildDayStrip(start, count, currentDate) {
    const out = [];
        ...
        out.push({
            ...
            mon: (() => {
                if (!p)
                    return "";
                try {
                    return new Intl.DateTimeFormat("en-GB", { timeZone: "UTC", month: "short" }).format(new Date(Date.UTC(p[0], p[1] - 1, p[2], 12)));
                }
                catch {
                    return "";
                }
 ...
 */
export function timeOptions(slots, currentDate, currentTime) {
    ...
    if (slots.date === currentDate && currentTime)
        out.push({ time: currentTime, current: true });
...
export function groupTimes(options) {
    if (options.length <= 8)
...
/* ── Shared frame ──────────────────────────────────────────────────────── */
const CLOSE_ICON = '<svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" ' +
    'stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"></path></svg>';
export function dialogFrameHTML(opts) {
    const id = `cw-dlg-${opts.kind}`;
    return (`<dialog class="cw-dialog cw-dialog--${opts.kind}" data-cw-dialog="${opts.kind}" aria-labelledby="${id}-title"` +
...
export function rescheduleSubline(treatment, venue, startIso) {
...
export function dayChipsHTML(days, selected) {
    return days
        .map((d) => `<button type="button" class="cw-rs-day${d.isCurrent ? " is-current" : ""}" data-cw-rs-day="${escapeHtml(d.date)}" ` +
...
function timeGridHTML(options, s) {
    const day = dateWords(s.date, "long");
    return groupTimes(options)
        .map((g) => `<div class="cw-rs-group">` +
...
export function reviewBarHTML(oldLabel, newLabel, busy = false) {
...
export function rescheduleDialogHTML(ctx, days, selected) {
...
export function cancelTitle(startIso, treatment) {
...
export function cancelBodyHTML(s, canReschedule, alert) {
...
export function cancelFootHTML(s, busy = false) {
    const label = busy ? "Cancelling…" : s ? s.confirmLabel : "Cancel booking";
    return (`<div class="cw-cx-actions">` +
