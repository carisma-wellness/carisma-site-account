 ...
 */
export const SSO_PAID_LANDING_PREFIXES = ["/lp/"];
/**
...
export function isPaidLanding(path, search, paidPrefixes = SSO_PAID_LANDING_PREFIXES) {
    if (paidPrefixes.some((p) => path.startsWith(p)))
        return true;
...
export function ssoProbeDecision(input) {
    ...
    if (has(cookie, "cw-sso-signout"))
        return trigger === "load" ? "signout" : null;
    ...
    if (has(cookie, "cw-sso-probed"))
        return null;
    if (trigger === "load" && isPaidLanding(path, input.search ?? "", input.paidPrefixes))
        return null;
    return "probe";
}
/**
...
export function runSsoProbe(env, trigger, opts = {}) {
    ...
    try {
        if (action === "probe") {
            ...
            if (has(doc.cookie, "cw-sso-signout"))
                return false;
...
const jsonForInlineScript = (v) => JSON.stringify(v).replace(/</g, "\\u003c");
/**
