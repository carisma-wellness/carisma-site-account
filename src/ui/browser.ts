...
export function mountAccountPortal(doc: MinimalDocument, opts: PortalMountOptions = {}): void {
              ...
              body: allFailed
                ...
                : // The record pages are built on the new system now; the
                  ...
                  `<div class="cw-rise">${bodyFor(
                    ...
                    { memberName, contactPhone: opts.contactPhone, siteBrand, siteOrigin: loc?.origin, invoicePay: opts.invoicePay === true },
                  )}</div>`,
            }),
          );
  ...
  if (!portalListBound.has(mount)) {
    ...
    (mount as unknown as Listenable).addEventListener?.("click", (ev: MinimalMouseEvent) => {
      ...
      if (action === "statement-pay") {
        ...
        if (!payId || (kind !== "package" && kind !== "invoice")) return;
        const done = setBusy(btn, "Opening payment…");
  ...
  if (invoiceReturn) {
    ...
    void postJson(fetchImpl, invoicePayConfirmCall(invoiceReturn.invoiceId, invoiceReturn.sessionId)).then(
      () => load(),
      () => load(),
    );
