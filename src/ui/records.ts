...
export function walletHTML(m: WalletModel): string {
  if (m.isEmpty) {
    return (
      `<p class="carisma-portal__empty">Nothing in your wallet yet — gift cards, treatment packages ` +
  ...
  const packs = m.packages.length
    ? `<h2 class="carisma-portal__eyebrow" style="margin-top:28px">Packages</h2>` +
      m.packages
        .map((p) => {
          const count =
            p.sessionsLeft !== null && p.sessionsTotal !== null
...
export function statementHTML(m: StatementModel): string {
  ...
  const line = (l: StatementLineView, showDue: boolean) => {
...
export function membershipHTML(m: MembershipModel): string {
  if (!m.hasMembership) {
    return (
      `<p class="carisma-portal__empty">You're not a member yet. ` +
  ...
  const facts =
    card("Plan", m.planName, statusWord) +
  ...
  const btn = (action: string, label: string) =>
    `<button type="button" class="carisma-portal__btn" data-cw-action="${action}" ` +
  ...
  const rest =
    `<p class="carisma-portal__note">To change the card that pays for this, or to cancel, ` +
