 ...
 */
export function resolveBrandLink(input: BrandLinkInput): string | null {
  if (!input.signedIn) return null;
  if (input.defaultPrevented || input.modified || input.targetBlank) return null;
  if (!input.brand) return null;
  if (typeof input.href !== "string" || input.href.length === 0) return null;
  ...
  if (url.hostname === "carisma-same-site.invalid") return null;
  // A Medical host is never rewritten, regardless of data-carisma-brand.
  if (MEDICAL_HOST_RE.test(url.hostname)) return null;
...
export function installBrandLinkInterceptor(doc: MinimalDocument, opts: InterceptorOptions): () => void {
  const onClick = (e: MinimalMouseEvent): void => {
    if (e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) {
      return;
    }
    const target = e.target;
    const anchor: MinimalElement | null =
  ...
  return () => {
    ...
    const anyDoc = doc as unknown as {
      removeEventListener?: (t: string, h: (e: MinimalMouseEvent) => void, c?: boolean) => void;
    };
