 ...
 * locked door.
 */
export function venueLocalToUtcIso(dateStr: string, timeStr: string, timeZone: string): string | null {
...
export function venueDateString(at: Date, timeZone: string): string {
  try {
...
export function buildSlotsModel(body: unknown, fallbackDate: string): SlotsModel {
  ...
  const raw = Array.isArray(o.slots) ? (o.slots as Array<Record<string, unknown>>) : [];
  return {
...
export function reschedulePickerHTML(m: SlotsModel, opts: { minDate: string }): string {
    ...
    : free.length
      ? `<div class="carisma-reschedule__times">` +
        free
          .map(
            (s) =>
              `<button type="button" class="carisma-reschedule__time" data-cw-slot="${escapeHtml(s.time)}">` +
  ...
  return (
    `<section class="carisma-reschedule" data-cw-qc="${RESCHEDULE_QC}" data-cw-tz="${escapeHtml(m.timeZone)}">` +
