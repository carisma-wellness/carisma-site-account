...
export function avatarUrlFromSession(body: unknown): string | null {
  if (!body || typeof body !== "object") return null;
  const b = body as Record<string, unknown>;
  const profile = (b.profile && typeof b.profile === "object" ? b.profile : b) as Record<string, unknown>;
 ...
 */
export function accountMarkPhotoHTML(url: string): string {
  return (
    `<img class="carisma-account-mark__photo" data-carisma-account-photo ` +
