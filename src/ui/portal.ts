...
import { readActions, statusLabel, type BookingActions } from "./bookingDetail.js";
...
export interface ApptCard {
  ...
  startMs: number;
  tile: { wk: string; day: string; mon: string } | null;
...
function parts(ms: number, opts: Intl.DateTimeFormatOptions): string {
  try {
...
function chipFor(status: string, upcoming: boolean, a: BookingActions, due: number): ApptCard["chip"] {
  if (status === "PENDING") return { label: statusLabel(status), tone: "warn" };
  if (upcoming && a.canPayBalance && due > 0) return { label: "Payment due", tone: "warn" };
  switch (status) {
    case "BOOKED":
    case "CONFIRMED":
...
export function buildApptCard(
  ...
  const balance = row.balance && typeof row.balance === "object" ? (row.balance as Record<string, unknown>) : null;
 ...
 */
export function buildApptCards(
  body: unknown,
  bucket: "upcoming" | "past",
  siteBrand = "",
): ApptCard[] {
  const rows =
    bucket === "upcoming" ? extractAppointmentList(body) : extractAppointmentBucket(body, "past").length
  ...
  const key = (c: ApptCard) => (Number.isFinite(c.startMs) ? c.startMs : bucket === "upcoming" ? Infinity : -Infinity);
...
const ARROW =
  '<svg class="cw-btn__arrow" aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" ' +
  'stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M13 6l6 6-6 6"></path></svg>';

function dateTile(c: ApptCard, large = false): string {
  const cls = `cw-date${large ? " cw-date--lg" : ""}${c.upcoming ? "" : " cw-date--past"}`;
  if (!c.tile) return `<span class="${cls}" aria-hidden="true"><span class="cw-date__day">·</span></span>`;
  return (
...
function chipHTML(c: ApptCard, extraClass = ""): string {
  return c.chip
    ? `<span class="cw-chip cw-chip--${c.chip.tone}${extraClass ? " " + extraClass : ""}">${escapeHtml(c.chip.label)}</span>`
    : "";
}

function spoken(c: ApptCard): string {
  return [c.service, c.longDay ? `on ${c.longDay}` : ""].filter(Boolean).join(" ");
}

/** The card's own buttons: from `actions`, and only from `actions`. */
function cardButtons(c: ApptCard, size: "sm" | "md" = "sm", primaryFirst = false): string[] {
  const a = c.actions;
  const cls = (primary: boolean) =>
    `cw-btn ${primary ? "cw-btn--primary" : "cw-btn--secondary"}${size === "sm" ? " cw-btn--sm" : ""}`;
  const out: string[] = [];
  const id = escapeHtml(c.id);
  if (a.canPayBalance && c.due > 0) {
    out.push(
      `<button type="button" class="${cls(primaryFirst && out.length === 0)}" data-cw-action="pay" data-cw-appt="${id}" ` +
        `aria-label="${escapeHtml(`Pay ${eur(c.due)} for ${spoken(c)}`)}">Pay ${escapeHtml(eur(c.due))}</button>`,
    );
  }
  if (c.upcoming && a.canReschedule) {
    out.push(
      `<button type="button" class="${cls(primaryFirst && out.length === 0)}" data-cw-action="reschedule" data-cw-appt="${id}" ` +
...
function reasonHTML(c: ApptCard): string {
  // Only for a visit still ahead: a past booking needs no explanation of why
  // it cannot be moved.
  if (!c.upcoming || c.actions.canReschedule || !c.actions.reason) return "";
...
function signOutLinks(): string {
  return (
    `<div class="cw-signout">` +
...
export function errorBlockHTML(what: string, phone = ""): string {
  const tel = phone.replace(/\s+/g, "");
  return (
    `<div class="cw-error cw-rise" role="alert">` +
...
export function skeletonHTML(view: PortalView): string {
  const what =
    view === "home" || view === "bookings" || view === "booking"
      ? "your bookings"
      : view === "details"
        ? "your details"
        : view === "refer"
...
const CALENDAR_ICON =
  '<svg aria-hidden="true" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" ' +
...
export function bookCtaLabel(siteBrand: string): string {
  const k = brandKey(siteBrand);
  if (k.includes("slimming")) return "Book your free body analysis";
  if (k.includes("pulse")) return "Book a club tour";
  if (k.includes("spa")) return "Book a spa day";
  if (k.includes("aesthetics") || k.includes("hair")) return "Book a free consultation";
  return "Book a treatment";
}

function emptyVisitsHTML(m: PortalModel, title: string, text: string): string {
  return (
...
function nextVisitHTML(c: ApptCard, now: Date): string {
  const a = c.actions;
  // One primary: Pay > Reschedule > Confirm. The next one down is secondary.
  const buttons: Array<{ action: string; label: string; aria: string }> = [];
  if (a.canPayBalance && c.due > 0) buttons.push({ action: "pay", label: `Pay ${eur(c.due)}`, aria: `Pay ${eur(c.due)} for ${spoken(c)}` });
  if (a.canReschedule) buttons.push({ action: "reschedule", label: "Reschedule", aria: `Reschedule ${spoken(c)}` });
  if (a.canConfirm && c.status !== "CONFIRMED") buttons.push({ action: "confirm", label: "Confirm I'm coming", aria: `Confirm you're coming to ${spoken(c)}` });
  const btns = buttons
    .slice(0, 2)
    .map(
      (b, i) =>
        `<button type="button" class="cw-btn ${i === 0 ? "cw-btn--primary" : "cw-btn--secondary"}" data-cw-action="${b.action}" ` +
  ...
  return (
    `<section class="cw-next cw-rise" aria-labelledby="cw-next-title">` +
 ...
 */
export function needsYou(m: PortalModel, excludeId = ""): NeedRow[] {
  ...
  for (const c of cards) {
    ...
    if (a.canReschedule && Number.isFinite(closes) && closes - now > 0 && closes - now < 48 * HOUR) {
      out.push({
        card: c,
        title: `Online changes close ${untilPhrase(a.rescheduleClosesAt as string, m.now)}`,
        sub: [c.service, when].filter(Boolean).join(" · "),
        action: "reschedule",
        label: "Reschedule",
        calm: true,
...
const WALLET_ICON =
  '<svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" ' +
...
function needsHTML(rows: NeedRow[]): string {
  if (!rows.length) return "";
  return (
    `<section class="cw-section cw-rise" aria-labelledby="cw-needs-title">` +
...
export function walletSummary(w: WalletModel | null): string {
  if (!w || w.isEmpty) return "";
  const bits: string[] = [];
  if (w.credit > 0) bits.push(`${eur(w.credit)} credit`);
  const live = w.giftCards.filter((g) => g.balance > 0);
  if (live.length) bits.push(`${live.length} gift card${live.length === 1 ? "" : "s"}`);
  const pack = w.packages.find((p) => p.sessionsLeft !== null && (p.sessionsLeft ?? 0) > 0);
  if (pack) {
    bits.push(
      pack.sessionsTotal !== null
...
function walletStripHTML(w: WalletModel | null): string {
  const text = walletSummary(w);
  if (!text) return "";
  return (
    `<a class="cw-strip cw-rise" href="/account/wallet">` +
...
export function overviewBodyHTML(m: PortalModel): string {
  ...
  if (!next) {
    ...
    return (
      emptyVisitsHTML(
        ...
        "This is where your visits, gift cards and membership will live.",
      ) +
      needs +
...
export function bookingsBodyHTML(m: PortalModel): string {
  const section = (id: string, title: string, cards: ApptCard[], count = true) =>
    `<section class="cw-section cw-rise" aria-labelledby="cw-${id}-title">` +
...
export function detailsBodyHTML(m: PortalModel): string {
  const fact = (label: string, value: string) =>
    `<div class="cw-fact"><dt class="cw-label">${label}</dt><dd class="cw-fact__value" ${M}>${escapeHtml(value || "—")}</dd></div>`;
  return (
    `<section class="cw-card cw-rise"><dl class="cw-facts">` +
    fact("Name", m.name) +
    fact("Mobile", m.phone) +
...
function nextLede(m: PortalModel): string {
  const n = m.upcomingCards.length;
  const next = m.upcomingCards[0];
  if (m.upcomingState === "failed") return "";
  if (!n) return "Nothing booked yet.";
  return `${n} upcoming${next?.shortDay ? ` · next on <strong>${escapeHtml(next.shortDay)}</strong>` : ""}`;
}

export function portalTitle(m: PortalModel): string {
  if (m.view === "bookings") return "Your bookings";
  if (m.view === "details") return "Your details";
