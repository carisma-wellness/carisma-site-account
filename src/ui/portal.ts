...
function nav(view: PortalView): string {
  const item = (href: string, label: string, id: PortalView) =>
    `<a href="${href}"${view === id ? ' aria-current="page"' : ""}>${label}</a>`;
  return (
...
export function accountPortalHTML(model: PortalModel): string {
  ...
  return (
    `<main class="carisma-portal" data-cw-qc="${PORTAL_QC}" data-cw-portal="${escapeHtml(model.view)}">` +
