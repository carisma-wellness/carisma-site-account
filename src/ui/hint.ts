...
export function readSignedInHint(cookie: string | null | undefined): boolean {
 ...
 */
export function readInitialsHint(cookie: string | null | undefined): string {
  const m = (cookie || "").match(
    new RegExp(`(?:^|;\\s*)${HINT_INITIALS}=([A-Za-z]{1,2})(?:;|$)`),
  );
