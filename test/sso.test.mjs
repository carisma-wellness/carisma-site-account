...
test("a successful callback stamps cw-known, clears cw-sso-seed and lifts cw-sso-off", async () => {
  ...
  const r = routes(fetchImpl);
  const start = await r.start(new Request(`${SITE}/api/auth/start?prompt=none&next=/x`));
  const state = new URL(start.headers.get("location")).searchParams.get("state");
  const txn = valueOf(cookieNamed(start, "cw_txn"));
  const res = await r.callback(
    new Request(`${SITE}/api/auth/callback?code=${randomToken(32)}&state=${state}`, { headers: { cookie: `cw_txn=${txn}` } }),
  );
  assert.equal(res.status, 302);
